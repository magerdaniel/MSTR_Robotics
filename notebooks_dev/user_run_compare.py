...
from mstr_robotics.prepare_ai_data import MstrToJson, ParseJson
from mstr_robotics.read_out_prj_obj import ReadGen
from mstr_robotics.redis_db import FetchItAll, RedisBiAnalysis, RedisMstrJson
...
i_json_comparator = JSONComparator()
...
class RunCompare:
    ...
    def search_update(self, conn, search_id):
        all_obj_d_l = {}
        search_result = i_mstr_api.run_mstr_search(conn=conn, search_id=search_id)
    ...
    def update_searches(self, searches_used_in_prp_d_l):
        i_redis_mstr_json.run_searches_redis(
            conn=self.conn,
    ...
    def run_comparison(self, play_compare_d):
        ...
        if play_compare_d["run_daily_update_fg"]:
            for pre in self.project_prefix:
                ...
                i_redis_mstr_json.save_obj_json_to_redis(
                    i_redis_bi_analysis=self.i_redis_bi_analysis,
                    conn=self.conn,
                    prefix_map=self.prefix_map,
        ...
        if obj_list_type == "search_id":
            ...
            all_obj_d_l = self.shortcut_folder(
                self.conn, self.project_prefix, play_compare_d["obj_list_type"]["shorcut_folder_id"]
            )
        ...
        all_org_objects_df = pd.DataFrame(
            i_fetch_it_all.fetch_all_objects_recursively(root_object_l=org_root_object_id_l, recursive_fg=recursive_fg)
        )
        ...
        if len(diff_d_l) > 0:
            ...
            diff_d["no_number_path"] = diff_d["path_list"].apply(i_json_comparator.remove_bracket_numbers)
            ...
            self.conn.select_project(comp_project_id)
            comp_obj_prop_d_l = i_mstr_api.get_proj_obj_by_id_l(
                conn=self.conn, obj_id_l=diff_df["comp_obj_id"].tolist()
            )
            comp_obj_prop_df = pd.DataFrame(comp_obj_prop_d_l)
            df = pd.merge(diff_df, org_obj_prop_df, left_on=["org_obj_id"], right_on=["id"], how="inner")
            df = pd.merge(df, comp_obj_prop_df, left_on=["comp_obj_id"], right_on=["id"], how="inner")

            print(df.columns)
            # Group by org_obj_key and aggregate both json_key_path and diff_type
            org_obj_diff_path_df = (
                df.groupby(["org_obj_key", "comp_obj_id", "id_x", "id_y"])
                .agg(
                    {
                        "name_x": max,
                        "obj_type_x": max,
            ...
            for _i, obj in org_obj_diff_path_df.iterrows():
                # Keep the full key including prefix (e.g., "mstr_dev:METRIC:12345")

                obj_d = {
                    "org_obj_key": obj["org_obj_key"],
                    "org_obj_info": {
                        "id": obj["id_x"],
                        "name": obj["name_x"],
