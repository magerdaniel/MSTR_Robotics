...
class RedisBiAnalysis:
    def __init__(
        ...
        self.redis_client = redis.Redis(
            host=host, port=port, db=db, username=username, password=password, decode_responses=decode_responses
        )
    ...
    def emergency_flush_db(self, confirm_phrase: str = None) -> dict:
        ...
        if confirm_phrase != "FLUSH_ALL_DATA":
            return {"status": "error", "message": "Must provide confirm_phrase='FLUSH_ALL_DATA' to proceed"}

        try:
            self.redis_client.flushdb()
            return {"status": "success", "message": "Database flushed successfully", "deleted_count": "all"}
        except Exception as e:
            return {"status": "error", "message": f"Failed to flush database: {e}"}
    ...
    def upload_key_value(self, key: str, value, data_type: str = "json") -> dict:
        ...
        try:
            ...
            return {"status": "error", "message": f"Failed to upload key '{key}': {e}", "key": key}
    ...
    def fetch_key_value(self, key: str, data_type: str = "json") -> dict:
        ...
        try:
            ...
            if not self.redis_client.exists(key):
                return {"status": "error", "message": f"Key '{key}' does not exist", "key": key, "value": None}
            ...
            if data_type == "json":
                ...
                return {"status": "error", "message": f"Unsupported data_type: {data_type}", "key": key, "value": None}
            ...
            return {"status": "error", "message": f"Failed to fetch key '{key}': {e}", "key": key, "value": None}
                ...
                try:
                    ...
                    for i, current_key in enumerate(batch_keys):
                        try:
                            ...
                            elif not recursive_fg and recursive_cnt == 0:
                                recursive_cnt = 1
                                child_keys = self.extract_child_object_keys(obj_def)
                                for child_key in child_keys:
                                    if child_key not in visited_objects:
...
class RedisMstrJson:
    ...
    def bld_redis_key(self, conn, object_id, env_prefix):
        ...
        search_d = {"projectIdAndObjectIds": [{"projectId": conn.project_id, "objectIds": [object_id]}]}
    ...
    def run_searches_redis(self, conn, i_redis_bi_analysis, prefix_map, env_prefix, searches_used_in_prp_d_l):
        ...
        for s in searches_used_in_prp_d_l:
            search_result = i_mstr_api.run_mstr_search(conn=conn, search_id=s["search_id"])
            obj_def = {"id": s["search_id"], "search_id": s["search_id"], "obj_type": 39}
            obj_def = i_mstr_to_json.create_obj_def_search_rel(
                obj_def=obj_def, search_obj_d_l=search_result["result"], prefix_map=prefix_map
            )
            ...
            obj_def["child_obj_d_l"] = self.bld_redis_child_key(
                env_prefix=env_prefix, child_obj_d_l=obj_def["child_obj_d_l"]
            )
    ...
    def save_obj_json_to_redis(self, conn, i_redis_bi_analysis, all_obj_d_l, prefix_map, env_prefix):
        ...
        for o in all_obj_d_l:
            try:
                ...
                obj_def = i_mstr_to_json.child_object_handler(
                    conn=conn, obj_def=obj_def, obj_type=o["type"], prefix_map=prefix_map
                )
                ...
                err_d_l.append(
                    {"id": o["id"], "type": o["type"], "subtype": o["subtype"], "name": o["name"], "error": str(e)}
                )
    ...
    def get_redis_prefix(self, subtype_text):
        prefix_map = {
            "report_grid_and_graph": "REPORT_DEFINITION",
