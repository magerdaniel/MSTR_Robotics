...
class AnswerPrompts:
    ...
    def bld_met_prp_ans(self, d_prp_ans_d, operator, filter_val_l):
        ...
        d_prp_ans_d["prp_ans_d"] = self.bld_exp_prp_met_ans(
            obj=d_prp_ans_d, operator=operator, filter_val_l=filter_val_l
        )
        return d_prp_ans_d
    ...
    def bld_att_element_prp_ans(self, d_prp_ans_d, filt_obj_rag_df, filter_val_l=None):
        ...
        filt_df = pd.merge(
            filt_obj_rag_df[["project_id", "prompt_id", "prp_subType", "object_id", "obj_type"]],
            filt_att_element_df[
                [
                    "project_id",
                    "prompt_id",
            ...
            left_on=["project_id", "object_id"],
            right_on=["project_id", "attribute_id"],
            how="inner",
        )

        filt_df = i_df_helper.clean_double_col(df=filt_df)
        filt_df.drop_duplicates(inplace=True)
        for _i, ans in filt_df.iterrows():
            ele_ans_JSON_l.append(str(ast.literal_eval(ans["ele_prp_ans"])))
            prompt_id = ans["prompt_id"]

        att_element_ans_d = i_prompts.frame_prp_ans(
            prompt_id=prompt_id, prp_type="ELEMENTS", prp_ans_JSON_l=ele_ans_JSON_l
        )
    ...
    def bld_exp_elem_prp_ans(self, d_prp_ans_d, filt_obj_rag_df, operator, filter_val_l=None):
        ...
        filter_val_df = self.attribute_form_elements_df[
            self.attribute_form_elements_df["element_val"].isin(filter_val_l)
        ]
        filt_df = pd.merge(
            filt_obj_rag_df[["project_id", "prompt_id", "prp_subType", "object_id", "obj_type"]],
            filter_val_df[
        ...
        att_exp_ans_d = {
            "att_id": d_prp_ans_d["object_id"],
            "att_form_id": form_id,
            "form_data_type": form_dataType,
            "operator": operator,
    ...
    def bld_att_qual_prp_ans(self, attribute, d_prp_ans_d, operator, filter_val_l):
        ...
        for _index, form in filt_obj_rag_df_sorted.iterrows():
            if int(form["display_form_nr"]) > 0:
                att_exp_ans_d = {
                    "att_id": form["attribute_id"],
                    "att_form_id": form["form_id"],
                    "form_data_type": form["REST_form_type"],
                    "operator": operator,
                    "filter_val_l": filter_val_l,
                }
    ...
    def AI_mstr_prp_page_ans(self, conn, vector_store, bi_request_d, rep_dos_id):
        ...
        try:
            ...
            for f in b_filter_d.keys():
                try:
                    ...
                    if f[:9] == "att_eleme":
                        ...
                        prp_filt_obj_rag_df = rep_dos_obj_prp_rel_df[
                            (rep_dos_obj_prp_rel_df["object_name"] == b_filter_d[f]["attribute"])
                            & (rep_dos_obj_prp_rel_df["prp_subType"].isin(["prompt_expression", "prompt_elements"]))
                        ]
                        for _i, obj in prp_filt_obj_rag_df.iterrows():
                            ...
                            if prp_subType == "prompt_elements":
                                prp_ans_d = self.bld_att_element_prp_ans(
                                ...
                                prp_ans_d = self.bld_exp_elem_prp_ans(
                                    d_prp_ans_d=d_prp_ans_d,
                                    filt_obj_rag_df=prp_filt_obj_rag_df,
                    ...
                    if f[:9] == "att_qual_":
                        ...
                        prp_ans_d = self.bld_att_qual_prp_ans(
                            attribute=b_filter_d[f]["attribute"],
                            d_prp_ans_d=d_prp_ans_d,
                            operator=b_filter_d[f]["operator"],
                    ...
                    if f[:9] == "metric_fi":
                        prp_filt_obj_rag_df = rep_dos_obj_prp_rel_df[
                            (rep_dos_obj_prp_rel_df["object_name"] == b_filter_d[f]["metric"])
                            & (rep_dos_obj_prp_rel_df["prp_subType"] == "prompt_expression")
                        ]
                        for _i, obj in prp_filt_obj_rag_df.iterrows():
                            d_prp_ans_d["prompt_id"] = obj["prompt_id"]
                        ...
                        prp_ans_d = self.bld_met_prp_ans(
                            d_prp_ans_d=d_prp_ans_d,
                            operator=b_filter_d[f]["operator"],
            ...
            bld_ai_obj_ans_prp_d_l = self.bld_ai_obj_ans_prp(obj_rel_df=rep_dos_obj_prp_rel_df, key_l=all_obj_key_l)
    ...
    def bld_ai_obj_ans_prp(self, obj_rel_df, key_l):
        ...
        prp_ans_d_l = (
            obj_prp_rag_df.groupby(["prompt_id"]).apply(lambda x: x[["id", "type"]].to_dict(orient="records")).to_dict()
        )
        obj_prp_ans_l = []
        ...
        for prp_ans_id in prp_ans_d_all:
            for prp in ai_prp_filt_ans_d:
                if prp_ans_id == prp["prompt_id"]:
                    if prp["prp_subType"] == "prompt_expression" and (
                        prp["obj_type"] == "attribute" or prp["obj_type"] == "12"
                    ):
                        prp_ans_l.append(
                            i_prompts.bld_prp_exp_d(
                                prompt_id=prp_ans_id, p_ans_d_j=prp_ans_d_all[prp_ans_id], operator="And"
                            )
                        )

                    if prp["prp_subType"] == "prompt_expression" and (
                        prp["obj_type"] == "metric" or prp["obj_type"] == "4"
                    ):
                        prp_ans_l.append(
                            i_prompts.frame_metric_exp_prp(prompt_id=prp_ans_id, p_ans_d_j=prp_ans_d_all[prp_ans_id])
                        )
    ...
    def save_AI_rep(self, conn, report_id, prompt_answ, ai_rep_name, ai_rep_folder_id, promptOption="static"):
        ...
        rep_id = i_rep.save_rep_inst(
            conn=conn,
            instance_id=instance_id,
            rep_name=ai_rep_name,
            save_mode="OVERWRITE",
