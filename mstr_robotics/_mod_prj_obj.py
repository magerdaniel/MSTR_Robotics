...
class BldShortCuts:
    ...
    def bld_short_cut(self, conn, folder_id, object_id, object_type, new_name, object_desc):
        ...
        short_cut_obj_rep = self.mstr_api.cr_short_cut(
            conn=conn, object_id=object_id, object_type=object_type, folder_id=folder_id
        )
        self.mstr_api.rename_object(
            conn=conn, object_id=short_cut_obj_rep.json()["id"], object_type=18, new_name=new_name, desc_str=object_desc
        )
    ...
    def run_short_cut_build(self, conn, project_id, short_cut_df, folder_id, df_key_cols_d=None):
        ...
        for _index, obj in short_cut_df.iterrows():
            ...
            try:
                if obj[df_key_cols_d["object_type"]] != str(18):
                    ...
                    print(
                        "Create short: "
                        + obj[df_key_cols_d["object_name"]]
                        + " of type: "
                        + str(obj[df_key_cols_d["object_type"]])
                    )
                    self.bld_short_cut(
                        conn=conn,
                        folder_id=folder_id,
                    ...
                    print(
                        f"Obect: {obj[2]} is a shortcut. MSTR does not allow short cuts o short cuts,"
    ...
    def clean_shortcut_folder(self, conn, folder_id):
        ...
        self._get_col_from_obj_l(dict_l=existing_obj_l, key_col="id")
