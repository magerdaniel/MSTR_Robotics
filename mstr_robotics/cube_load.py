...
if __name__ == "__main__":
    ...
    # Chain definition lives in cube_load_sample.json — copy it, replace the
    # placeholder GUIDs with your own cube IDs, and point CUBE_LOAD_PLAN at it.
    _plan_path = Path(os.environ.get("CUBE_LOAD_PLAN", Path(__file__).with_name("cube_load_sample.json")))
    with open(_plan_path, "r", encoding="utf-8") as _fh:
        load_json_d_l = json.load(_fh)

    for _item in load_json_d_l:
        if _item.get("project_id") == "PROJECT_GUID":
            _item["project_id"] = _default_project_id
