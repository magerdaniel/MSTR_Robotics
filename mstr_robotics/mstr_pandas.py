...
class DfHelper:
    ...
    def flag_folder_df(self, flag_path_d_l, obj_df, path="path_ids", merge_key_l=None, flag_folder_id="flag_folder_id"):
        if merge_key_l is None:
            merge_key_l = ["project_id", "id"]
        ...
        for _folder_id in flag_path_df["flag_folder_id"]:
            flag_path_df = pd.DataFrame.from_dict(flag_path_d_l)
            unique_str = flag_path_df[flag_folder_id].unique()
            matching_rows_df = obj_df[obj_df[path].str.contains("|".join(unique_str))]
            flaged_obj_df = pd.merge(
                matching_rows_df,
                flag_path_df,
                left_on=matching_rows_df[path].str.extract(f"({'|'.join(unique_str)})")[0],
                right_on=flag_folder_id,
                how="outer",
            )
