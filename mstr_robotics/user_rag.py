...
class ChatBot:
    def run_chat_msg(self, msg_t, sKey, model, vector_store, max_tokens, temperature):
        ...
        json_t = MstrOpenAi().call_open_AI(
            sKey=sKey, messages=messages, max_tokens=max_tokens, temperature=temperature, model=model
        )
        ...
        json_f = MstrOpenAi().call_open_AI(
            sKey=sKey, messages=messages, max_tokens=max_tokens, temperature=temperature, model=model
        )
    ...
    def filter_RAG_l(self, msg_t, key_word_l, msg_filter_t):
        messages = [
            ...
            {"role": "system", "content": "When filtering a list, please ensure, that you use In od NotIn as operator"},
            {"role": "system", "content": "Ensure, that the out pu is a dictionary consiting of keywords"},
            {"role": "system", "content": "top_n or bottom_n is part of the filter statement"},
            {"role": "system", "content": f"keywrods are: {key_word_l} "},
            {
                "role": "system",
            ...
            {"role": "system", "content": "Set brackets to split multiple filter criteria"},
            {"role": "system", "content": "Ignore keywords, if they are not relevant for filtering"},
            {"role": "system", "content": f"keywrods are: {key_word_l} "},
            {"role": "user", "content": f"{msg_filter_t}"},
        ]
        return messages

    def split_AI_msg(self, msg_t, key_word_l):
        messages = [
            {"role": "system", "content": "You will be provided unstructured BI report request."},
            {
                "role": "system",
            ...
            {"role": "system", "content": "Please use the key template and filter split"},
            {"role": "system", "content": "Set brackets to split multiple filter criteria"},
            {"role": "system", "content": "Please include in the JSON file, the filter critria as string"},
            {"role": "system", "content": f"keywrods are: {key_word_l} "},
            {"role": "user", "content": f"{msg_t}"},
        ]
        return messages
...
class Perplexity:
    ...
    def rag_sys_template(self, key_word_l, att_elem_str, bi_obj_str):
    ...
    def merge_AI_ans_d(self, merge_AI_ans_d_l):
        m_AI_ans_fin_d = {"attributes": [], "metrics": [], "filter": {}, "question": ""}
