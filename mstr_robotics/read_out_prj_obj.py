...
from mstr_robotics.mstr_classes import MdSearches, MstrGlobal
...
class ReadOutHierarchy:
    ...
    def read_out_hier_att_df(self, conn, proj_hier_l):
        ...
        for hier in proj_hier_l:
            hier_att_l = i_mstr_api.get_hier_att(conn=conn, hier_id=hier["id"])
    ...
    def read_out_sys_hier(self, conn):
        ...
        proj_hier_l = i_md_searches.search_for_type_l(
            conn=conn, obj_l=["14"], name="System Hierarchy", info_level="base"
        )
        sys_hier_att_df = self.read_out_hier_att_df(conn=conn, proj_hier_l=proj_hier_l)
    ...
    def read_out_hier_df(self, conn, proj_hier_l=None):
        ...
        # proj_hier_l = None
        if proj_hier_l is None:
            proj_hier_l = i_md_searches.search_for_type_l(conn=conn, obj_l=["14"], name="System Hierarchy")

        hier_att_df = self.read_out_hier_att_df(conn=conn, proj_hier_l=proj_hier_l)
...
class ReadTableDef:
    ...
    def runreadout(self, conn, *args, **kwargs):
        ...
        for t in tbl_id_l:
            t_cols_l = self.get_tbl_col(base_url=conn.base_url, conn=conn, table_id=t["tbl_id"])
            ...
            try:
                ...
                for form in att["forms"]:
                    if "isFormGroup" not in list(form.keys()):
                        ...
                        all_att_maps_d["REST_form_type"] = i_prompts.get_exp_prp_data_type(
                            baseFormType=form["dataType"]["type"]
                        )
...
class ReadSchema:
    ...
    def table_mappings(self, conn, *args, **kwargs):
        ...
        tbl_att_fct_df = pd.merge(
            table_df[["project_id", "table_id", "column_id", "physicalTable_id", "physicalTable_name"]],
            att_form_exp_df[["project_id", "table_id", "column_id", "attribute_id", "form_expressionId"]],
            on=["project_id", "table_id", "column_id"],
            how="left",
        )
        ...
        schema_df_d = {
            "tbl_att_fct_df": tbl_att_fct_df,
            "att_form_exp_df": att_form_exp_df,
            "fact_exp_df": fact_exp_df,
            "table_df": table_df,
        }
...
class ReadGen:
    ...
    def chk_by_obj_type(self, conn, obj_type_l, mtdi_id=None, *args, **kwargs):
        ...
        obj_l = i_md_searches.search_for_used_in_obj_direct(conn=conn, obj_l=obj_l, dpn_fg=True, *args, **kwargs)

        obj_depn_df = pd.DataFrame.from_dict(obj_l)
        return obj_depn_df

    def get_proj_obj_def_by_id_l(self, conn, obj_id_l):
        obj_def_d_l = []
        obj_id_d_l = i_mstr_api.get_proj_obj_by_id_l(conn=conn, obj_id_l=obj_id_l)
        for o in obj_id_d_l:
            obj_def_d_l.append(
                self.get_obj_def(conn=conn, object_id=o["id"], obj_type=o["obj_type"], obj_sub_type=o["subtype"])
            )
        return obj_def_d_l
    ...
    def get_obj_def(self, conn, object_id, obj_type=None, obj_sub_type=None):
        ...
        try:
            if str(obj_type) == "43":
            ...
            elif str(obj_type) == "48":
                obj_def = i_mstr_api.get_derived_element(conn=conn, derived_element_id=object_id)

            elif str(obj_type) == "10":
                obj_def = i_mstr_api.get_prompt_def(conn=conn, prompt_id=object_id)
            elif str(obj_type) == "39":
                obj_def = i_mstr_api.get_se

            elif str(obj_sub_type) == "14081":
                obj_def = i_mstr_api.get_dossier_def(conn=conn, dossier_id=object_id)

            elif str(obj_type) == "58":
                obj_def = security_filters.get_security_filter(
                    connection=conn, id=object_id, show_expression_as="tokens"
                ).json()
                ...
                if obj_sub_type not in [
                    ...
                    print("nicht gemappt")
                    print("sub_object_id:" + str(obj_sub_type))
                    print("object_id: " + str(object_id))
                    print("obj_type: " + str(obj_type))
                    obj_not_mapped_d = {"object_id": object_id, "obj_type": obj_type, "obj_sub_type": obj_sub_type}
                    self.obj_not_mapped_d_l.append(obj_not_mapped_d.copy())

        except Exception as e:
            print("Error reading object definition")
            print("object_id: " + str(object_id))
            print("obj_type: " + str(obj_type))
            ...
            self.obj_read_error_d_l.append(obj_read_error_d.copy())

            # obj_def = {"error": str(e), "object_id": object_id, "obj_type": obj_type, "obj_sub_type": obj_sub_type}

        return obj_def

    def find_type_subtype(self, type_sub_type_int):
        try:
            type_sub_type_int = int(type_sub_type_int)
            mstr_ai_obj_type = ObjectTypes(type_sub_type_int)._name_

            if "UNDEFINED_TYPE" in mstr_ai_obj_type:
                mstr_ai_obj_type = ObjectSubTypes(type_sub_type_int)._name_
        except Exception:
            mstr_ai_obj_type = "unknown"

        return mstr_ai_obj_type

    def child_obj_type_handler(self, subtype):

        if subtype in ["table_partition_wh"]:
            mstr_ai_obj_group = "TABLE"
        elif subtype in ["role_transformation"]:
            mstr_ai_obj_group = "ROLE_TRANSFORMATION"
        elif subtype in ["attribute_form_system"]:
            mstr_ai_obj_group = "ATTRIBUTE"
        elif subtype in ["db_role"]:
            mstr_ai_obj_group = "DB_ROLE"
        elif subtype in ["prompt_expression"]:
            mstr_ai_obj_group = "PROMPT"
        elif subtype in ["prompt"]:
            mstr_ai_obj_group = "PROMPT"
        elif subtype in ["dimension_user"]:
            mstr_ai_obj_group = "DIMENSION_USER"
        elif subtype in ["metric"]:
            mstr_ai_obj_group = "METRIC"
        elif subtype in ["prompt_objects"]:
            mstr_ai_obj_group = "PROMPT_OBJECTS"
        elif subtype in ["filter"]:
            mstr_ai_obj_group = "FILTER"
        elif subtype in ["physical_table"]:
            mstr_ai_obj_group = "TABLE"
        elif subtype in ["md_security_filter"]:
            mstr_ai_obj_group = "MD_SECURITY_FILTER"
        elif subtype in ["attribute"]:
            mstr_ai_obj_group = "ATTRIBUTE"
        elif subtype in ["dimension_user_hierarchy"]:
            mstr_ai_obj_group = "HIERARCHY"
        elif subtype in ["drill_map"]:
            mstr_ai_obj_group = "DRILL_MAP"
        elif subtype in ["logical_table"]:
            mstr_ai_obj_group = "TABLE"
        elif subtype in ["db_table_pmt"]:
            mstr_ai_obj_group = "TABLE"
        elif subtype in ["fact"]:
            mstr_ai_obj_group = "FACT"
        elif subtype in ["report_grid"]:
            mstr_ai_obj_group = "REPORT"
        elif subtype in ["14081"]:
            mstr_ai_obj_group = "DASHBOARD"
        else:
            mstr_ai_obj_group = "UNKNOWN"
        return mstr_ai_obj_group

    def get_obj_id_by_path(self, conn, path_str, top_folder_id="D3C7D461F69C4610AA6BAA5EF51F4125"):
        """
        ...
        for segment in segments[1:]:
            folder_contents = i_mstr_global.get_folder_obj_l(conn=conn, folder_id=current_folder_id)
...
class ReadPrompts:
    ...
    def read_obj_prp_def(self, conn, prompt_def_d):
        ...
        if "predefinedObjects" in prompt_def_d["question"].keys():
            for obj in prompt_def_d["question"]["predefinedObjects"]:
                ...
                p_obj_row_d["obj_prp_ans"] = i_prompts.bld_obj_prp_json(
                    object_id=obj["objectId"], object_type=obj["subType"]
                )
                p_obj_row_d["object_name"] = obj["name"]
            ...
            if (
                ...
                and prompt_def_d["question"]["listAllHierarchies"]
            ):
                print("allHierPrompt")
            else:
                ...
                self.prompt_obj_l.append(p_obj_row_d.copy())
                self.prp_search_obj_l.append(
                    {"prompt_id": p_obj_row_d["prompt_id"], "type": 10, "search_id": p_obj_row_d["object_id"]}.copy()
                )
    ...
    def _add_att_to_obj_l(self, conn, p_row_d):
        ...
        p_obj_row_d["obj_prp_ans"] = i_prompts.bld_obj_prp_json(object_id=p_row_d["object_id"], object_type="attribute")
        p_obj_row_d["object_name"] = p_row_d["object_name"]
        self.prompt_obj_l.append(p_obj_row_d.copy())
    ...
    def read_ele_prp_def(self, conn, prompt_def_d):
        ...
        p_row_d["object_name"] = prompt_def_d["question"]["attribute"]["name"]
    ...
    def obj_prp_search_dpn(self, conn, search_id_l):
        prp_obj_rel_l = []

        for prp_obj in search_id_l:
            search_id = prp_obj["search_id"]

            try:
                search_resp = i_mstr_api.run_mstr_search(conn, search_id=search_id)
                ...
                for obj in results:
                    ...
                    prp_obj_rel_d["prompt_id"] = prp_obj["prompt_id"]
                    prp_obj_rel_d["object_id"] = obj["id"]
                    prp_obj_rel_d["obj_type"] = str(obj["type"])
    ...
    def run_read_out_prp_def(self, conn, prompts_l=None):
        ...
        if len(self.prp_search_obj_l) > 0:
            ...
            prompt_search_l = self.obj_prp_search_dpn(conn=conn, search_id_l=self.prp_search_obj_l)
            self.prompt_obj_l.extend(prompt_search_l)

        return {
            "prompt_def_l": self.prompt_def_l,
            "prompt_ele_l": self.prompt_ele_l,
            "prompt_obj_l": self.prompt_obj_l,
            "prompt_exp_l": self.prp_search_exp_l,
        }
    ...
    def add_prp_def_to_cube(self, conn, cube_name="prompt_def", cube_id=None, cube_folder_id=None, force=False):
        ...
        i_cube.upload_cube_mult_table(
            conn=conn,
            tbl_upd_dict=prompt_d_l,
            mtdi_id=cube_id,
            cube_name=cube_name,
...
class ReadReport:
    ...
    def read_rep_quick_filt(self, conn, report_id, show_advanced_properties="false"):
        rep_def = i_rep.get_report_all_def(
            conn=conn, report_id=report_id, show_advanced_properties=show_advanced_properties
        )
        filter_text = ""
        report_limit_text = ""
    ...
    def read_reports(self, conn, report_id_l, load_d=None):
        if load_d is None:
            load_d = {}
        rep_head_l = []
        rep_avail_obj_l = []
        ...
        for report_id in report_id_l:
            try:
                ...
                rep_def_err_d = {
                    "load_d": load_d,
                    "rep_def": rep_def,
                    "grid_obj_d": grid_obj_d,
                    "pageBy_obj_d": pageBy_obj_d,
                    "avail_obj_d": avail_obj_d,
                    "err_msg": str(e),
                }
    ...
    def bld_rep_def_df(self, rep_head_l, rep_grid_obj_l, rep_avail_obj_l):
        ...
        rep_head_df.rename(
            columns={"type": "rep_type", "type_bez": "rep_type_bez", "subtype": "rep_subtype"}, inplace=True
        )
        ...
        rep_def_df = pd.merge(
            ...
            rep_avail_obj_df[
                ["project_id", "report_id", "object_id", "object_name", "obj_type", "form_id", "form_name"]
            ],
            left_on=["project_id", "report_id"],
            right_on=["project_id", "report_id"],
            how="inner",
        )
        ...
        rep_def_df = pd.merge(
            ...
            right_on=["project_id", "report_id", "object_id", "form_id"],
            how="left",
        )
        rep_def_df = i_df_helper.clean_double_col(df=rep_def_df)
        return rep_def_df
...
class ReadCube:
    def read_cube_model_header(self, conn, cube_id, cube_head_d=None):
        cube_all_def_d = i_mstr_api.get_cube_all_def(
            conn=conn,
            cube_id=cube_id,
        )
    ...
    def fetch_cube_att_elem(self, conn, cube_id, limit_val=10000, *args, **kwargs):
        ...
        att_elem_d_l = []
        offset_val = 0
        resp = i_mstr_api.get_v2_cube_instance(
            conn=conn, cube_id=cube_id, offset_val=offset_val, limit_val=limit_val, *args, **kwargs
        )
        # url = f'{conn.base_url}/api/v2/cubes/{cube_id}/instances?offset={offset_val}&limit={limit_val}'
        # resp = conn.post(url)
        cube_def = resp.json()

        for row in cube_def["definition"]["grid"]["rows"]:
            if row["type"] == "attribute":
                rag_att_elem_d = {}
                rag_att_elem_d["project_id"] = conn.project_id
                rag_att_elem_d["attribute_id"] = row["id"]
                rag_att_elem_d["attribute_name"] = row["name"]
                at_ele_l = i_mstr_api.get_cube_att_eleme_d_l(conn=conn, cube_id=cube_id, att_id=row["id"])
                for el in at_ele_l:
                    rag_att_elem_d["ans_id"] = el["id"].split(";;")[0] + ";" + row["id"]
                    rag_att_elem_d["element_val"] = el["id"].split(";;")[1]
                    rag_att_elem_d["ele_prp_ans"] = {
                        "id": rag_att_elem_d["ans_id"],
                        "name": rag_att_elem_d["element_val"],
                    }

                    att_elem_d_l.append(rag_att_elem_d.copy())
    ...
    def fetch_cube_form_elements(self, conn, cube_id, limit_val=10000, *args, **kwargs):
        rag_att_form_l = []
        offset_val = 0
        row_count = 1
        while row_count > 0:
            ...
            for att in cube_def["definition"]["grid"]["rows"]:
                ...
                for form in att["forms"]:
                    ...
                    for element in att["elements"]:
                        rag_att_form_d["element_val"] = element["formValues"][form_nr]
    ...
    def read_cube_att_elem_d_l(self, conn, cube_list_l, *args, **kwargs):
        all_cube_element_l = []
        for cube in cube_list_l:
            cube_element_l = self.fetch_cube_att_elem(conn=conn, cube_id=cube["id"], *args, **kwargs)
    ...
    def read_cube_att_form_exp_val_l(self, conn, cube_list_l, *args, **kwargs):
        all_cube_element_l = []
        for cube in cube_list_l:
            cube_element_l = self.fetch_cube_form_elements(conn=conn, cube_id=cube["id"], *args, **kwargs)
            all_cube_element_l.extend(cube_element_l)
        return all_cube_element_l
