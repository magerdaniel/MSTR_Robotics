...
class read_gen():
    ...
    def get_obj_def(self,conn, object_id, obj_type=None, obj_sub_type=None):
        ...
        try:
            if str(obj_type)=="43":
                obj_def = transformations.get_transformation(connection=conn, id=object_id, show_expression_as="tokens").json()
                ...
                obj_def = security_filters.get_security_filter(
                                    connection=conn, id=object_id,  
                                    show_expression_as="tokens").json()
            ...
            obj_read_error_d={"error":str(e), "object_id":object_id, "obj_type":obj_type, "obj_sub_type":obj_sub_type}
    ...
    def read_obj_prp_def(self,conn,prompt_def_d):
        ...
        if "predefinedObjects" in prompt_def_d["question"].keys():
            ...
            if ("expressionType" in prompt_def_d.keys() and
                ...
                self.prompt_obj_l.append(p_obj_row_d.copy())
                self.prp_search_obj_l.append({"prompt_id": p_obj_row_d["prompt_id"], "type": 10,"search_id": p_obj_row_d["object_id"]}.copy())
        ...
        for row in cube_def["definition"]["grid"]["rows"]:
            if row["type"] == "attribute":
                ...
                for el in at_ele_l:
                    ...
                    rag_att_elem_d["ele_prp_ans"]={"id": rag_att_elem_d["ans_id"],"name": rag_att_elem_d["element_val"]}
