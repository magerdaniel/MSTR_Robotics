...
class MstrGlobal:
    ...
    def get_obj_from_sh_fold(self, conn, folder_id):
        ...
        for obj_d in obj_l:
            if obj_d["type"] == 18:
                ...
                obj_det_d["subtype"] = obj_d["target_info"]["subtype"]
                obj_det_d["subtypebez"] = ObjectSubTypes(obj_d["target_info"]["subtype"]).name
    ...
    def get_object_info(self, conn, object_id, type):
        return self.i_api_obj.get_object_info(
            connection=conn, id=object_id, object_type=type, project_id=conn.headers["X-MSTR-ProjectID"]
        )
    ...
    def get_object_info_d(self, conn, object_id, type, *args, **kwargs):
        ...
        # simply removed
        obj_d = self.i_api_obj.get_object_info(
            connection=conn, id=object_id, object_type=type, project_id=conn.headers["X-MSTR-ProjectID"]
        )
    ...
    def _delete_object(self, conn, existing_obj_l):
        for o in existing_obj_l:
            self.i_api_obj.delete_object(connection=conn, id=o["id"], object_type=o["type"])
        return
    ...
    def get_obj_name(self, conn, report_id, type):
        ...
        return self.i_api_obj.get_object_info(conn=conn, object_id=report_id, type=type).json()["name"]
    ...
    def bld_obj_d(self, conn, obj_d, *args, **kwargs):
        ...
        obj_row_d = {}
        path = self.bld_obj_path(
            fld_d=obj_d["ancestors"], proj_name=self.i_mstr_api.get_project_name(conn=conn, project_id=conn.project_id)
        )
        if self.i_str_func._get_first_x_chars(str_=str(path["path_name"]), i=16) != "\\System Objects\\" and obj_d.get(
            "type"
        ) in [1, 2, 3, 4, 6, 8, 10, 11, 12, 13, 14, 15, 18, 23, 34, 55]:
            obj_row_d = {
...
class MdSearches:
    ...
    def __init__(self, run_prop_d=None, dpn_prefix="dpn_"):
    ...
    def search_for_used_in_obj_direct(
        self, conn, obj_l, dpn_fg=True, info_level="base_path", count_only_fg=True, *args, **kwargs
    ):
        ...
        kwargs = {
            ...
            "count_only_fg": count_only_fg,
            "dpn_fg": dpn_fg,
            "mstr_search": {"uses_one_of": False, "obj_val_uses_object": obj_l, "uses_recursive": False},
        }
    ...
    def _run_search(self, conn, *args, **kwargs):
        ...
        try:
            serach_inst = self.brow.store_search_instance(
                conn, project_id=conn.project_id, **kwargs["mstr_search"]
            ).json()
            search_obj_l.extend(
                self._paging_search_l(
                    conn=conn, run_prop_d=self.run_prop_d, dpn_prefix=None, serach_inst=serach_inst, *args, **kwargs
                )
            )
        ...
        for obj in obj_l:
            ...
            if obj_type not in search_obj_l:
                ...
                mstr_search_d = self.i_str_func.replace_val_by_prefix(
                    dict=kwargs["mstr_search"].copy(), prefix="obj_val_", obj_val=obj_type
                )
                ...
                if serach_inst["totalItems"] > 0:
                    search_obj_l.extend(
                        self._paging_search_l(
                            ...
                            dpn_prefix=self.dpn_prefix,
                            *args,
                            **kwargs,
                        )
                    )
                elif not kwargs["count_only_fg"]:
                    search_obj_l.append(
                        run_prop_d
                        | self._bld_dummy_info(dpn_prefix=self.dpn_prefix, run_prop_d=run_prop_d, *args, **kwargs)
                    )
                elif kwargs["count_only_fg"]:
                    search_obj_l.append(run_prop_d | {"count_obj": 0})

                if cube_to_loop_d is not None and len(search_obj_l) > 50:
                    if i == 0:
    ...
    def _add_search_to_cube(self, conn, search_obj_l, updatePolicy, cube_to_loop_d, mtdi_id):
        ...
        mtdi_id = self.cube_it.upload_cube_mult_table(
            conn,
            mtdi_id=mtdi_id,
            tbl_upd_dict=load_upd_d_l,
            cube_name=cube_to_loop_d["cube_name"],
            folder_id=cube_to_loop_d["folder_id"],
            force=cube_to_loop_d["force"],
        )
        print(str(len(search_obj_l)) + " rows load to cube")
        return mtdi_id

    def _paging_search_l(self, conn, serach_inst, dpn_prefix, run_prop_d, max_count=100, limit=50, *args, **kwargs):
        search_obj_l = []
        offset = 0
        count_obj = 0
        ...
        while serach_inst["totalItems"] > offset:
            try:
                search_result_d = self.brow.get_search_results(
                    connection=conn,
                ...
                full_obj_info_l = self._extract_info_from_search(
                    conn=conn,
                    dpn_prefix=dpn_prefix,
                    run_prop_d=run_prop_d,
                    search_result_d=search_result_d.json(),
                    *args,
                    **kwargs,
                )

                if not kwargs["count_only_fg"] or not kwargs["dpn_fg"]:
    ...
    def _extract_info_from_search(self, conn, search_result_d, dpn_prefix, run_prop_d, *args, **kwargs):
        ...
        for obj_d in search_result_d:
            try:
                if "info_level" not in kwargs.keys():
                ...
                if kwargs["info_level"] == "base":
                    ...
                    obj_row_d = MstrGlobal().get_object_info_d(
                        conn=conn,
                        object_id=obj_d["id"],
                        type=obj_d["type"],
                        dpn_prefix=dpn_prefix,
