...
def parse_ds_obj(obj_def_d):
    ...
    for ds in obj_def_d["datasets"]:
        ...
        for obj in ds.get("availableObjects", []):
            placement = []
            if obj["id"] in row_ids:
                placement.append("rows")
            if obj["id"] in col_ids:
                placement.append("columns")
            if obj["id"] in pageby_ids:
                placement.append("pageBy")
...
def find_dataset_relationships(obj_def_d):
    ...
    attr_df = df[df["obj_type"] == "attribute"][["dataset_id", "dataset_name", "obj_id", "obj_name"]].drop_duplicates()
    ...
    if form_rows.empty:
        ...
        if cx:
            entry["custom_extensions"] = [
                _mstr_ext({"obj_id": first["obj_id"], "obj_type": first["obj_type"], "forms": []})
            ]
        return [entry]
                ...
                _mstr_ext(
                    {
                        ...
                        "dataType": r["dataType"],
                        "baseFormCategory": r["baseFormCategory"],
                    }
                )
            ]
...
def _build_osi_dataset(ds_rows, cx=False):
    ...
    if cx:
        dataset["custom_extensions"] = [
            _mstr_ext({"dataset_id": first["dataset_id"], "dataset_name": first["dataset_name"]})
        ]
    if fields:
...
def build_osi_semantic_model(conn, dossier_id, custom_extensions=False):
    ...
    obj_def_d = i_read_gen.get_obj_def(
        conn=conn, object_id=dossier_id, obj_type=OBJ_TYPE_DOSSIER, obj_sub_type=OBJ_SUBTYPE_DOSSIER
    )
    df = parse_ds_obj(obj_def_d)
    datasets = [
        _build_osi_dataset(grp.reset_index(drop=True), cx=cx) for _, grp in df.groupby("dataset_id", sort=False)
    ]
    relationships = _build_osi_relationships(obj_def_d, cx=cx)
    metrics = _build_osi_metrics(df, cx=cx)

    sm = _cm({"name": dossier_id, "datasets": datasets}, name_comment=obj_def_d.get("name", ""))
    if relationships:
...
def _build_selector_filter(grp, key_to_name, cx=False):
    ...
    filt = _cm({"name": first["sel_filt_key"], "type": osi_type}, name_comment=first["sel_filt_name"])
    if target_keys:
...
def _semantic_ref(obj_rows, ref_type, cx=False) -> list:
    ...
    if form_rows.empty or ref_type == "metric":
        r = _cm({"name": first["object_id"], "type": ref_type}, name_comment=first["object_name"])
        if cx:
            r["custom_extensions"] = [
                _mstr_ext(
                    {
...
def _build_visualization(viz_rows, cx=False):
    ...
    viz = _cm(
        {"name": first["visual_key"], "type": _map_viz_type(first["visualizationType"])},
        name_comment=first["visual_name"],
    )
    if cx:
        viz["custom_extensions"] = [
            _mstr_ext(
                {
...
def _build_page(page_rows, page_sel_df, key_to_name, cx=False):
    ...
    vizs = [
        _build_visualization(grp.reset_index(drop=True), cx=cx)
...
def _build_chapter(chap_rows, chap_filt_df, chap_sel_df, key_to_name, cx=False):
    ...
    for page_key, page_rows in chap_rows.groupby("page_key", sort=False):
        page_sel = chap_sel_df[chap_sel_df["page_key"] == page_key] if not chap_sel_df.empty else pd.DataFrame()
...
def build_osi_dashboard(doss_hier_l, filt_sel_d, semantic_model_name, custom_extensions=False):
    ...
    key_to_name = (
        hier_df.drop_duplicates("visual_key").set_index("visual_key")["visual_name"].to_dict()
    ...
    for dossier_id, doss_rows in hier_df.groupby("dossier_id", sort=False):
        ...
        for chap_key, chap_rows in doss_rows.groupby("chapter_key", sort=False):
            ...
            chapters.append(_build_chapter(chap_rows.reset_index(drop=True), chap_filt, chap_sel, key_to_name, cx=cx))
        db = _cm(
            {"name": dossier_id, "semantic_model": semantic_model_name, "chapters": chapters},
...
def read_out_obj(conn, search_instance_resp, search_obj):
    ...
    ol = [
        {"id": o["id"], "type": o["type"], "subtype": o["subtype"]}
        for o in osi_list
        if o["subtype"] in [3072, 768, 1024, 3840, 3328]
    ]
    ol.append(search_obj)
    return [
        i_read_gen.get_obj_def(conn=conn, object_id=o["id"], obj_type=o["type"], obj_sub_type=o["subtype"]) for o in ol
    ]
