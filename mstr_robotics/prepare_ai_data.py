...
class SortMstrJson:
    ...
    def sort_json_lists_by_hash(self, json_obj, obj_type, ignore_keys=None):
        """
        Recursively sort ALL lists in JSON by hash of their content (bottom-up approach)

        Args:
            json_obj: JSON object to process
            obj_type: Type of the object (for legacy compatibility)
            ignore_keys: List of keys to ignore when creating hash (e.g., ["predicateId","versionId","dateModified","dateCreated"])

        Returns:
            JSON object with all lists sorted by content hash
        """
        ...
        def remove_ignored_keys(obj, ignore_keys):
            """
            Create a copy of object with ignored keys removed (for hash calculation only)
            """
        ...
        def get_hash(item):
            """
            Calculate hash of an item for sorting
            Ignores keys specified in ignore_keys list
            """
            # Remove ignored keys before hashing
            cleaned_item = remove_ignored_keys(item, ignore_keys)

            # Convert to JSON string with sorted keys for consistent hashing
            try:
                json_str = json.dumps(cleaned_item, sort_keys=True, default=str)
                return hashlib.md5(json_str.encode()).hexdigest()
            except Exception:
                # Fallback to string representation if JSON serialization fails
                return hashlib.md5(str(cleaned_item).encode()).hexdigest()

        def sort_recursive(obj, current_path=None):
            """
            Recursively process the JSON structure with path tracking
            Sorts from lowest level up (bottom-up)
            """
            ...
            if isinstance(obj, dict):
                ...
                # First, recursively process each item in the list (go deeper)
                for i, item in enumerate(obj):
                    # Include list index in path
                    item_path = current_path + [f"[{i}]"]
                    obj[i] = sort_recursive(item, item_path)

                # Then, sort THIS list by hash of each item's content
                # This ensures we sort after all nested content is already sorted
                try:
                    # Sort the list by hash
                    obj.sort(key=lambda x: get_hash(x))
                except Exception as e:
                    print(f"Warning: Could not sort list at path {'.'.join(str(p) for p in current_path)}: {e}")
    ...
    def sort_json_lists_by_keys(
        ...
        """
        DEPRECATED: Use sort_json_lists_by_hash instead
        ...
        """
        # Redirect to hash-based sorting for better consistency
...
class MstrToJson:
    ...
    def child_object_handler(self, conn, obj_def, obj_type, prefix_map):
        ...
        if obj_type == 3:
            ...
            obj_def["ai_report_filter"] = self.report_filter_objects(obj_def_d=obj_def, prefix_map=prefix_map)
        elif obj_type == 55:
    ...
    def replace_random_id(self, obj_def, key_to_replace, path_l=None):
        """
        ...
        def replace_ids_recursive(obj, key_to_replace, current_path=None):
    ...
    def sort_all_lists(self, data, ignore_keys):
        """
        Recursively sort all lists in a JSON structure.
        Lists are sorted by string representation of their elements,
        excluding specified keys from dict elements.
        """

        def create_sort_key(element, ignore_keys):
            """Create a string sort key for a list element."""
            if isinstance(element, dict):
                # Filter out ignore keys
                filtered_dict = {k: v for k, v in element.items() if k not in ignore_keys}
                # Convert to sorted string representation
                return str(sorted(filtered_dict.items()))
            else:
                # For non-dict elements, convert to string
                return str(element)

        def sort_recursive(obj):
            """Recursively process and sort the data structure."""
            if isinstance(obj, dict):
                # Recurse into dict values
                return {k: sort_recursive(v) for k, v in obj.items()}
            elif isinstance(obj, list):
                # First recurse into list elements to handle nested structures
                processed_list = [sort_recursive(item) for item in obj]
                # Then sort the list by the sort key
                sorted_list = sorted(processed_list, key=lambda x: create_sort_key(x, ignore_keys))
                return sorted_list
            else:
                # Return primitives as-is
                return obj

        return sort_recursive(data)
    ...
    def add_dataset_obj_type(self, conn, obj_def):
        ...
        for child_obj in child_obj_d_l:
            if len(child_obj["keyPath"]) == 3 and child_obj["keyPath"][0] == "datasets":
                ...
                search_d = {"projectIdAndObjectIds": [{"projectId": conn.project_id, "objectIds": [child_obj["key"]]}]}
    ...
    def report_filter_objects(self, obj_def_d, prefix_map):
        ...
        child_d_l = self.i_parse_json.extract_specific_key_value_pairs(
            data=obj_def_d["dataSource"]["filter"], path_fg=False, json_type="rep_filter", prefix_map=prefix_map
        )
        prompt_element_d_l = []
        ...
        try:
            ...
            if obj_def_d["type"] != 39:  # search objects
                obj_def = i_read_gen.get_obj_def(
                    conn=conn, object_id=obj_def_d["id"], obj_type=obj_def_d["type"], obj_sub_type=obj_def_d["subtype"]
                )
                if "error" not in obj_def.keys():
                    if obj_def_d["subtype"] == "256":
                        obj_def = self.replace_random_id(
                            obj_def, key_to_replace="predicateId", path_l=["qualification"]
                        )
...
class ParseJson:
    ...
    def extract_specific_key_value_pairs(
        self,
        data,
        target_keys=("id", "key", "objectId"),
        all_pairs=None,
        path_fg=True,
        json_type="all",
        prefix_map=None,
            ...
            for unit_no, unit in enumerate(json_obj_def["dataSource"]["dataTemplate"]["units"]):
                for child_no, child in enumerate(unit["limit"]["tree"]["children"]):
                    if "predicateId" in child:
                        del json_obj_def["dataSource"]["dataTemplate"]["units"][unit_no]["limit"]["tree"]["children"][
                            child_no
                        ]
